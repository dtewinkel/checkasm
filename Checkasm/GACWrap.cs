...
namespace CheckAsm.ManagedGAC
{
    ...
    {
        public static List<AsmData> GetAssembliesInGac(BackgroundWorker bgWorker, double maxLibs)
        {
            List<AsmData> ret = new List<AsmData>();
            double count = 0;
            ...
            do 
            {
                ...
                if (asmName != null)
                {
                    try
                    {
                        string path = QueryAssemblyInfo(asmName);
                        AsmData asm = new AsmData(asmName, path);
                        asm.AssemblyFullName = asmName;
                        asm.Validity = AsmData.AsmValidity.Valid;
                        ret.Add(asm);
                    }
                    catch (Exception ex)
                    {
                        Trace.WriteLine("Failed to read information for GAC Assembly " + asmName);
                        Trace.WriteLine(ex.ToString());
                    }
                    count++;
                    if (bgWorker != null)
                    {
                        bgWorker.ReportProgress(Convert.ToInt32(count * 100 / maxLibs));
                    }
                }
            } while (!String.IsNullOrEmpty(asmName));
            return ret;
        }
